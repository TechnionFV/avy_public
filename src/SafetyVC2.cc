...
SafetyVC2::SafetyVC2(Aig_Man_t *pCircuit, bool fPropagate, bool flemmaAbs,
                     AvyTrace *db)
    : m_TraceMan(db), m_propagate(fPropagate), m_lemmaAbs(flemmaAbs) {
  init(pCircuit);
}

void SafetyVC2::init(Aig_Man_t *pCircuit) {
  ...
  // -- construct Bad
  Aig_Man_t *pBad = nullptr;
  if (gParams.tr0) {
    pBad = Aig_ManDupSinglePo(&*m_Circuit, 0, false);
  } else {
    pBad = Aig_ManDupSinglePo(&*aig, 0, false);
  }
  Aig_ManRebuild(&pBad);
  m_Bad = aigPtr(pBad);
