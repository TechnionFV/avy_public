...
int Pdr_ManPushClauses( Pdr_Man_t * p )
{
    ...
    Vec_VecForEachLevelStartStop( p->vClauses, vArrayK, k, 1, kMax )
    {
      ...
      Vec_PtrForEachEntry(Pdr_Set_t *, vArrayK, pCubeK, j) {
        ...
        if (RetValue2 == -1) return -1;
        if (!RetValue2) continue;

        {
          Pdr_Set_t *pCubeMin;
          pCubeMin = Pdr_ManReduceClause(p, k, pCubeK);
          if (pCubeMin != NULL) {
            //                Abc_Print( 1, "%d ", pCubeK->nLits -
            //                pCubeMin->nLits );
            Pdr_SetDeref(pCubeK);
            pCubeK = pCubeMin;
          }
...
int Pdr_CNFizationInt(Pdr_Man_t* p, Pdr_Man_t* pGlobal, int nFrame)
{
    p->pPars->fMonoCnf = 1;
    int fPrintClauses = 0;
    Pdr_Set_t * pCube = NULL;
    ...
    // If we are CNFizing the first interpolant, no need for more than 1 frame.
    // Represented by fNotInit
    p->pPars->nFrameMax = 2;
    p->iOutCur = 0;
    if (nFrame > 0)
    {
        p->pPars->nFrameMax = 3;
        Pdr_ManCreateSolver(p, ++k);
        Pdr_ManSetPropertyOutput( p, 0 );
        // Add the clauses for the first bucket.
        // Since the deltas are being managed by the buckets, must add everything
        // starting at bucket nFrame-1 and up to MaxLevel.
        Vec_VecForEachLevelStart( pGlobal->vClauses, vVec, i, nFrame)
        {
            Vec_PtrForEachEntry( Pdr_Set_t *, vVec, pCube, j )
            {
                Vec_VecPush(p->vClauses, 1, pCube);
                p->nCubes++;
                Pdr_ManSolverAddClause(p, 1, pCube);
            }
        }
        p->iUseFrame = ABC_INFINITY;
        // push clauses into this timeframe
...
//                    printf( "Dropping output %d.\n", p->iOutCur );
        ...
        if ( p->pPars->nFrameMax && k >= p->pPars->nFrameMax )
        {
            ...
            // Add the clauses generated by CNFization to the proper bucket
            /*int nStart = (nFrame > 0) ? 1 : 0;
            vVec = Vec_VecEntry(pGlobal->vClauses, nStart);

            Vec_PtrForEachEntry( Pdr_Set_t *, vVec, pCube, j )
            {
                Vec_VecPush(pGlobal->vClauses, nFrame+1, pCube);
                pGlobal->nCubes++;
                for (int i = 1; i <= nFrame; i++)
                    Pdr_ManSolverAddClause(pGlobal, i, pCube);
            }*/
...
int Pdr_Push(Pdr_Man_t *p)
{
    return Pdr_ManPushClauses(p);
}
