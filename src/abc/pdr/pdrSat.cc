...
//    pSat = sat_solver_new();
    ...
    // XXX We now have frames in which property is not known.
    // XXX This should only be set of the original solver asserted the property
...
int Pdr_ManCheckCube( Pdr_Man_t * p, int k, Pdr_Set_t * pCube, Pdr_Set_t ** ppPred, int nConfLimit )
{
    ...
    if ( pCube == NULL ) // solve the property
    {
        ...
        Limit = sat_solver_set_runtime_limit( pSat, Pdr_ManTimeLimit(p) );
        //sat_solver_store_write(pSat, "sat.cnf");
