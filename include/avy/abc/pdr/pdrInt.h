...
struct Pdr_Set_t_
{
    ...
    unsigned    kind   : 1;
    unsigned    unused : 10;
    int         nLits  : 26;       // num flop literals
    unsigned    nDepth : 10;
    unsigned    nAge   : 10;
    unsigned    fQuip  : 1 ;
    unsigned    nOrigF : 10;
...
static inline void Pdr_ManSetCallbackParam(Pdr_Man_t* p, void* pParm)
{
    p->pCallbackParam = pParm;
}

static inline void Pdr_ManSetCallback(Pdr_Man_t* p, void (*callback)(int, void*))
{
    p->callback = callback;
}
...
extern int             Pdr_CNFizationInt(Pdr_Man_t* p, Pdr_Man_t* pGlobal, int nFrame);
extern int             Pdr_Push(Pdr_Man_t* p);
